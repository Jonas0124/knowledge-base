...
use diesel::prelude::*;
use crate::config::app_err::{business_err, AppErr};

pub async fn login_service(req: UserLoginRequest) -> Result<String, Box<dyn Error>> {
    let mut connection = db_connection().get().unwrap();

    // 2. 获取 user 信息
    let mut res = user.filter(email.eq(&req.username).or(username.eq(&req.username)))
        .first::<User>(&mut connection)
        .ok();
    let Some(res_user) = res else {
        return business_err(AppErr::CUSTOM(50001, "用户不存在"));
    };
    if req.password != res_user.password {
        return business_err(AppErr::CUSTOM(50002, "密码错误"));
    }
